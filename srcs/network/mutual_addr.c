...
/*   Updated: 2025/02/11 16:33:28 by ymanchon         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	setup_client(char *address, t_core *core)
{
	...
	inet_pton(AF_INET, address,
		(in_addr_t *)&core->network.tcp.sock.sin_addr.s_addr);
	core->network.tcp.sock.sin_family = AF_INET;
	core->network.tcp.sock.sin_port = htons(PORT);
	if (connect(core->network.tcp.fd,
			(struct sockaddr *)&core->network.tcp.sock,
...
void	setup_server(t_core *core)
{
	...
	uselezz = TRUE;
	core->network.tcp.fd = socket(AF_INET, SOCK_STREAM, 0);
	setsockopt(core->network.tcp.fd, SOL_SOCKET, SO_REUSEADDR,
		&uselezz, sizeof(int));
	core->network.tcp.sock.sin_family = AF_INET;
	core->network.tcp.sock.sin_port = htons(PORT);
	core->network.tcp.sock.sin_addr.s_addr = INADDR_ANY;
	if (bind(core->network.tcp.fd, (struct sockaddr *)&core->network.tcp.sock,
			sizeof(struct sockaddr_in)) == -1)
	{
		close(core->network.tcp.fd);
	...
	core->network.tcp.com = accept(core->network.tcp.fd,
			(struct sockaddr *)&core->network.tcp.sock,
			&socklen);
