...
/*   Updated: 2025/01/15 15:31:57 by mkhoury          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	triangle1_x(t_screenp points[3], t_core *core, t_face face)
{
	...
	if ((points[2].x - points[1].x) > 0)
		...
		step = -1;
	delta.y = (float) (points[2].y - points[1].y) / (points[2].x - points[1].x);
	delta.z =(float) abs(points[2].y - points[1].y);
	while (point.x != points[2].x)
	{
		point.x = points[1].x + i * step;
		point.y = points[1].y + (int) (delta.y * ((float)i));
		draw_line_texture((t_screenp []){points[0], point}, face, core, (float)i / delta.z);
		i++;
	}
}
